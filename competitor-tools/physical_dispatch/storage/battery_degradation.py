...
class Degradation:
    def __init__(self, soc_array, temp_array, current_array, times_array, t_start, storage_df):
        ...
        self.cycle_life = storage_df.loc[:,'cycle_life'].values
    ...
    def calculate_cost_cyc(self, idx, display=True):
        ...
        self._cost_cyc = ((delta_t * cost_EoL) / ((1+1/efficiency) * cycle_life * charge_cap )) * np.linalg.norm(current,1)
        #                 (  hrs   *    $    ) / (  1              *    1       *   A*hrs   )   * A
    ...
    def calculate_cost_therm(self, idx, display=True):
        ...
        self._cost_therm = ((self.dt*3600*self.dt_interval * self.C_EoL[idx] * K_T * self.resistances[idx]) / self.therm_cap[idx] ) * np.linalg.norm(self.currents[idx],2)**2
        # Ad-hoc multiplier to get closer to target cost
        self._cost_therm /= 10
        if display:
    ...
    def calculate_cost_soc(self, idx, display=True):
        ...
        self._cost_soc = (self.dt_interval * self.C_EoL[idx] * K_S * self.soc)
        # Ad-hoc multiplier to get closer to target cost
        self._cost_soc /= 5
        if display:
...
def test_one_day():
    ...
    for test in ['ConSoC_Temp20', 'ConSoC_Temp30', 'ConSoC_Temp40',
        ...
        battery_deg = Degradation(soc_array, temp_array, current_array, times_array, times_array[0], resource_str)
